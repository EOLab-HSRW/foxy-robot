...
install_ros2_from_debs() {
  ...
  info "Installing ROS 2 ${distro^} desktop and development tools."

  sudo apt-get update
  sudo apt-get install -y locales software-properties-common curl ca-certificates

  if ! locale charmap 2>/dev/null | grep -qi 'UTF-8'; then
    info "Configuring a UTF-8 locale."
  ...
  success "ROS 2 ${distro^} installed."
}

install_common_tools() {
  info "Installing build and repository tools."
  sudo apt-get update
...
initialize_rosdep() {
  if [[ ! -f /etc/ros/rosdep/sources.list.d/20-default.list ]]; then
    ...
    info "rosdep is already initialized."
  fi
}

configure_gazebo_repository() {
  info "Configuring the Gazebo package repository."
...
EOF
  ...
  success "Fortress-oriented packages removed."
}

install_harmonic_for_humble() {
  remove_humble_fortress_conflicts
  info "Installing the ROS 2 Humble / Gazebo Harmonic integration packages."
...
prepare_workspace() {
  ...
  if [[ "$ROS_DISTRO_SELECTED" == "humble" ]]; then
    ...
    info "Importing Humble simulation source dependencies."
...
build_simulation_workspace() {
  ...
  info "Installing dependencies for the simulation package closure only."
  if [[ "$ROS_DISTRO_SELECTED" == "humble" ]]; then
    rosdep install \
      -r \
      -y \
  ...
  info "Building foxy_bringup_sim and its workspace dependencies."
  colcon build \
  ...
  popd > /dev/null
  success "The simulation workspace built successfully."
}

main() {
  local expected_ros=""

  info "Foxy robot simulation installer"
